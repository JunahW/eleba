							...
							<tr onmouseover="this.style.backgroundColor = 'white'"
								...
								<td style="CURSOR: hand; HEIGHT: 22px" align="center"
									width="10%"><c:if test="${business.state==1}">激活</c:if> <c:if
										...
										test="${business.state==2}">审核</c:if></td>
			...
			<tr align="center">
				<td colspan="7">第${pageBean.currPage}页/共${ pageBean.totalPage}页
					<c:if test="${pageBean.currPage != 1}">
						<a
							href="${ pageContext.request.contextPath }/root/business/listActiveUsers.action?currPage=1">首页</a>
					</c:if> <c:if test="${pageBean.currPage >= 1}">
						<a
							href="${ pageContext.request.contextPath }/root/business/listActiveUsers.action?currPage=${pageBean.currPage-1}">上一页</a>|
						</c:if> <c:if test="${pageBean.currPage < pageBean.totalPage}">
						<a
							href="${ pageContext.request.contextPath }/root/business/listActiveUsers.action?currPage=${pageBean.currPage+1}">下一页</a>|
							</c:if> <c:if test="${pageBean.currPage != pageBean.totalPage}">
						<a
							href="${ pageContext.request.contextPath }/root/business/listActiveUsers.action?currPage=${pageBean.totalPage}">尾页</a>|
							</c:if>
				</td>
			</tr>
		</TBODY>
