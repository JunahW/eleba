...
var shop_detail_triangle = document
		.getElementsByClassName("shop_detail_triangle");
var shop_detail_triangle_white = document
		.getElementsByClassName("shop_detail_triangle_white");
for (let i = 0, shop_len = shop_item.length; i < shop_len; i++) {
	shop_item[i].onmouseenter = function() {
		if (i % 4 == 0) {// 外卖细节框的方向在右边
			...
			shop_detail_triangle_white[i].classList
					.add("shop_detail_triangle_white_right");

		} else {
			shop_detail[i].classList.add("detail_left");
			shop_detail_triangle[i].classList.add("shop_detail_triangle_left");
			shop_detail_triangle_white[i].classList
					.add("shop_detail_triangle_white_left");
		}
...
sidebar_tabs.onclick = function() {
	var html = "";
	$.post("/eleba/cart/showCart.action", function(result) {
		var html="";
		console.log(result);
		...
		for ( var i in arr) {
			...
			html+="<li class='list_item'><span class='food_name'>"+item.product.pname+
			"</span><span class='quantity'> <span class='sub'>-</span> <input class="+
			"'quan_input' type='text' value='"+item.count+"'> <span class='plus'>+</span>"+
			"</span> <span class='price'>"+item.product.price+"</span></li>"
...
//	清空购物车
	...
	$.get("/eleba/cart/clearCart.action", function(result){},'json');
