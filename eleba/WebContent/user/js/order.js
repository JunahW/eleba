...
for (let i = 0, len = order_sidebar_item.length; i < len; i++) {
	order_sidebar_item[i].onclick = function() {
		...
		if (order_item[1].style.display == "inline-block") {
			/*$.get("/buyer/addr/list.ation", function(result) {
				alert(result);
			});*/
			alert("223");
		} else {
...
var delete_fun = function() {
	...
	var location_con_item = document
			.getElementsByClassName("location_con_item");
	var location_verify_box = document
			.getElementsByClassName("location_verify_box");
	...
	for (let i = 0, len = delete_btn.length; i < len; i++) {
		delete_btn[i].onclick = function() {
			...
			verify_yes[i].onclick = function() {
				location_con_item[i].parentNode
						.removeChild(location_con_item[i]);
				delete_fun();
