	...
	<form action="${pageContext.request.contextPath }/user/register.action"
		...
		<div class="box">
			<input class="box_in user" placeholder="用户名" name="username"
				value="${user.username}"> <input class="box_in password"
				placeholder="密码" value="${user.password}" name="password"
				type="password"> <input class="box_in name" placeholder="姓名"
				name="name" value="${user.name}"> <input
				class="box_in email" placeholder="email" name="email"
				value="${user.email}"> <input class="box_in phone"
				placeholder="手机" name="telephone" value="${user.telephone}">
			<input class="box_in date" placeholder="年/月/日" name="birthday">
